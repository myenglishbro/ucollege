  ...
  if (!submitted) {
    return (
      <div style={{ background: bgMain }} className="min-h-screen flex flex-col items-center p-6">
        <button onClick={resetQuiz} className="self-start mb-4" style={{ color: accent }}>← Volver</button>
        <div className="w-full max-w-2xl bg-white p-8 rounded-2xl shadow-md">
          <div className="flex justify-between items-center mb-4">
            <h2 style={{ color: accent }} className="text-2xl font-bold">{test.title}</h2>
            <span style={{ color: accent }} className="font-medium">⏱️ {timeLeft}s</span>
          </div>
          <AnimatePresence>
            {feedback === 'correct' && (
  ...
  return (
    <div style={{ background: bgMain }} className="min-h-screen flex flex-col items-center p-6">
      ...
      <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-md text-center">
        ...
        {score === total ? (
          nextIndex < testsData.tests.length ? (
            <>
            ...
            <p className="text-green-600 font-bold">¡Completado! 🎉</p>
          )
        ) : (
          <button onClick={() => setSubmitted(false)} className="w-full py-2 bg-gray-300 text-gray-800 rounded-lg">
            Reintentar
          </button>
        )}
      </div>
